...
import edu.uci.ics.hyracks.imru.elastic.swap.SwapChildrenRequest;
import edu.uci.ics.hyracks.imru.elastic.swap.SwapTargetRequest;
...
public class DynamicAggregation<Model extends Serializable, Data extends Serializable> {
    ...
    boolean waitForAggregation() throws Exception {
        ...
        long swapTime = System.currentTimeMillis()
                + so.parameters.maxWaitTimeBeforeSwap;
        while (true) {
            ...
            synchronized (so.aggr.aggrSync) {
                ...
                for (int id : so.aggr.incomingPartitions) {
                    if (unfinishedPartition < 0
                            && !so.aggr.isPartitionFinished(id)
        ...
        if (isRoot) {
            ...
            if (model == null) {
                ...
                so.imruSpec.updateReceive(so.curPartition, 0,
                        ...
                        recvQueue);
                byte[] infoData = IMRUSerialize.serialize(info);
            ...
            SerializedFrames.serializeToFrames(so.imruContext, frame,
                    ...
                    so.aggregatedResult, so.curPartition,
                    so.aggr.targetPartition, so.curPartition, so.imruContext
                            .getNodeId()
                            + " reduce "
                            + so.curPartition
                            + " "
    ...
    void performSwap() throws Exception {
        ...
        while (true) {
            ...
            if (so.aggr.swappingTarget >= 0
                ...
                so.aggr.failedReason = "recv"
                        + so.aggr.failedReasonReceivedSize;
            } else {
                synchronized (so.aggr.aggrSync) {
